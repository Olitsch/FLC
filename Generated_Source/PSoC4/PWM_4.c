...
void PWM_4_Init(void)
{
    ...
    /* Set other values from customizer */
        ...
        #if (PWM_4__COUNT_DOWN == PWM_4_TC_COUNTER_MODE)
            PWM_4_WriteCounter(PWM_4_TC_PERIOD_VALUE );
        #endif  /* (PWM_4__COUNT_DOWN == PWM_4_TC_COUNTER_MODE) */
...
void PWM_4_WriteCompare(uint32 compare)
{
        ...
        uint32 currentMode;
    #endif /* (PWM_4_CY_TCPWM_V2) */

    #if (PWM_4_CY_TCPWM_V2)
        currentMode = ((PWM_4_CONTROL_REG & PWM_4_UPDOWN_MASK) >> PWM_4_UPDOWN_SHIFT);

        if (PWM_4__COUNT_DOWN == currentMode)
        {
            PWM_4_COMP_CAP_REG = ((compare + 1u) & PWM_4_16BIT_MASK);
        }
        else if (PWM_4__COUNT_UP == currentMode)
        {
            PWM_4_COMP_CAP_REG = ((compare - 1u) & PWM_4_16BIT_MASK);
        }
        else
...
uint32 PWM_4_ReadCompare(void)
{
        ...
        if (PWM_4__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_4_COMP_CAP_REG - 1u) & PWM_4_16BIT_MASK);
        }
        else if (PWM_4__COUNT_UP == currentMode)
        {
            regVal = ((PWM_4_COMP_CAP_REG + 1u) & PWM_4_16BIT_MASK);
        ...
        {
            regVal = (PWM_4_COMP_CAP_REG & PWM_4_16BIT_MASK);
        }
        return (regVal);
    #else
...
void PWM_4_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (PWM_4__COUNT_DOWN == currentMode)
        {
            PWM_4_COMP_CAP_BUF_REG = ((compareBuf + 1u) & PWM_4_16BIT_MASK);
        }
        else if (PWM_4__COUNT_UP == currentMode)
        {
            PWM_4_COMP_CAP_BUF_REG = ((compareBuf - 1u) & PWM_4_16BIT_MASK);
...
uint32 PWM_4_ReadCompareBuf(void)
{
        ...
        if (PWM_4__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_4_COMP_CAP_BUF_REG - 1u) & PWM_4_16BIT_MASK);
        }
        else if (PWM_4__COUNT_UP == currentMode)
        {
            regVal = ((PWM_4_COMP_CAP_BUF_REG + 1u) & PWM_4_16BIT_MASK);
        ...
        {
            regVal = (PWM_4_COMP_CAP_BUF_REG & PWM_4_16BIT_MASK);
        }
        return (regVal);
    #else
...
*   Values:
*     - PWM_4_CMD_CAPTURE    - Trigger Capture command
*     - PWM_4_CMD_RELOAD     - Trigger Reload command
*     - PWM_4_CMD_STOP       - Trigger Stop command
*     - PWM_4_CMD_START      - Trigger Start command
*
* Return:
*  None
*
