...
void PWM_2_Init(void)
{
    ...
    /* Set other values from customizer */
        ...
        #if (PWM_2__COUNT_DOWN == PWM_2_TC_COUNTER_MODE)
            PWM_2_WriteCounter(PWM_2_TC_PERIOD_VALUE );
        #endif  /* (PWM_2__COUNT_DOWN == PWM_2_TC_COUNTER_MODE) */
...
void PWM_2_WriteCompare(uint32 compare)
{
        ...
        uint32 currentMode;
    #endif /* (PWM_2_CY_TCPWM_V2) */

    #if (PWM_2_CY_TCPWM_V2)
        currentMode = ((PWM_2_CONTROL_REG & PWM_2_UPDOWN_MASK) >> PWM_2_UPDOWN_SHIFT);

        if (PWM_2__COUNT_DOWN == currentMode)
        {
            PWM_2_COMP_CAP_REG = ((compare + 1u) & PWM_2_16BIT_MASK);
        }
        else if (PWM_2__COUNT_UP == currentMode)
        {
            PWM_2_COMP_CAP_REG = ((compare - 1u) & PWM_2_16BIT_MASK);
        }
        else
...
uint32 PWM_2_ReadCompare(void)
{
        ...
        if (PWM_2__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_2_COMP_CAP_REG - 1u) & PWM_2_16BIT_MASK);
        }
        else if (PWM_2__COUNT_UP == currentMode)
        {
            regVal = ((PWM_2_COMP_CAP_REG + 1u) & PWM_2_16BIT_MASK);
        ...
        {
            regVal = (PWM_2_COMP_CAP_REG & PWM_2_16BIT_MASK);
        }
        return (regVal);
    #else
...
void PWM_2_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (PWM_2__COUNT_DOWN == currentMode)
        {
            PWM_2_COMP_CAP_BUF_REG = ((compareBuf + 1u) & PWM_2_16BIT_MASK);
        }
        else if (PWM_2__COUNT_UP == currentMode)
        {
            PWM_2_COMP_CAP_BUF_REG = ((compareBuf - 1u) & PWM_2_16BIT_MASK);
...
uint32 PWM_2_ReadCompareBuf(void)
{
        ...
        if (PWM_2__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_2_COMP_CAP_BUF_REG - 1u) & PWM_2_16BIT_MASK);
        }
        else if (PWM_2__COUNT_UP == currentMode)
        {
            regVal = ((PWM_2_COMP_CAP_BUF_REG + 1u) & PWM_2_16BIT_MASK);
        ...
        {
            regVal = (PWM_2_COMP_CAP_BUF_REG & PWM_2_16BIT_MASK);
        }
        return (regVal);
    #else
...
*   Values:
*     - PWM_2_CMD_CAPTURE    - Trigger Capture command
*     - PWM_2_CMD_RELOAD     - Trigger Reload command
*     - PWM_2_CMD_STOP       - Trigger Stop command
*     - PWM_2_CMD_START      - Trigger Start command
*
* Return:
*  None
*
